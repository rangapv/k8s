...
kubegist() {
  ...
  if [[ ( $rc -eq 0 ) ]]
  then
  echo "---" | sudo tee -a kube-adm-calico-agnostic.yaml > /dev/null
  echo "kind: KubeletConfiguration" | sudo tee -a kube-adm-calico-agnostic.yaml > /dev/null
  echo "apiVersion: kubelet.config.k8s.io/v1beta1" | sudo tee -a kube-adm-calico-agnostic.yaml > /dev/null
  echo "cgroupDriver: systemd" | sudo tee -a kube-adm-calico-agnostic.yaml > /dev/null
  fi
...
kubeinit() {
	...
	init1=$( sudo kubeadm init --config=./kube-adm-calico-agnostic.yaml | sudo tee ./flag.txt)
